...
//TODO XPI: When you use Data annotation equals and hashCode are generated with all fields, causing different side effects. For persistent entities. It includes in equals/hashCode all fields including collections and ID, causing many potential issues with performance and side effects.
/*TODO-2: Humm, yes, it's interesting. In my case - I will use @Getter/@Setter here and @Data for Event (where equals()/hashCode() are important as soon as events are mapped as element collection (not one-to-many which in turn is 'todo')
...
public class Trace
{
  ...
  //TODO XPI: It is better to use more natual and suitable mapping and then apply transformation to DTO. This connection represent relation between 2 persistent entities, not just element collection. For such purpose relational mapping should be used by design.
  @ElementCollection
  ...
  //TODO XPI: ElementCollection works bad on updates and is used for very simple cases like set/list of primitives or strings.
  @ElementCollection
  ...
  {
    return TraceRepresentation.of(paymentId, headerMap, created, updated);
  }
}
